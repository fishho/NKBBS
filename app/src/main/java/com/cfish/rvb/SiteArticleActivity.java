...
import com.cfish.rvb.adapter.SHeaderAdapter;
...
public class SiteArticleActivity extends AppCompatActivity implements View.OnClickListener {

    private String s_a_id,formatImg,uid;
    private RecyclerView replyRv;
    private RequestParams params,paramsUp,paramsPost;
    private SHeaderAdapter hAdapter;
...
//        });
        ...
        this.s_a_id = getIntent().getExtras().getString("s_a_id");
...
//                        .show();
    ...
    private void getData() {
        ...
        params.add("type", "get_site_article");
        params.add("s_a_id", this.s_a_id);
        //params.add("page", "1");
//        AsyncHttpClient client  = new AsyncHttpClient();
        PersistentCookieStore cStore = new PersistentCookieStore(this);
        //client.setCookieStore(cStore);
        HttpUtil.post(CommonData.siteURL, params, new TextHttpResponseHandler() {
            @Override
...
//                }
                ...
                if (resp.getString("status").equals("0")) {
                    ...
                    SiteArticle article = details.getArticle();

                    //another way to get gid
                    //g_id = article.getGid();
                    Log.d("Dfish","g_id another way"+g_id);

                    List<SiteReply> replyList = details.getReply();
...
//                lik_cnt.setText(article.getFav_num());
                    hAdapter = new SHeaderAdapter(SiteArticleActivity.this, article, author, replyList);
                    hAdapter.setOnItemClickListener(new SHeaderAdapter.OnRecyclerViewItemClickListener() {
                        @Override
                        public void onItemClick(View v, String data, int flag) {
                            ...
                            switch (flag) {
                                ...
                                case 2:
                                case 3:
                                    intent.setClass(SiteArticleActivity.this, UserInfoActivity.class);
                                    localBundle.putString("user_id", s[0]);
                                    localBundle.putString("title", s[1]);
                                    intent.putExtras(localBundle);
                                    SiteArticleActivity.this.startActivity(intent);
                                    break;
                                case 4:
                                    p_g_r_id = "";
                                    Log.d("Dfish", "p_g_r_id" + p_g_r_id);
                    ...
                    replyRv.setLayoutManager(new LinearLayoutManager(SiteArticleActivity.this));
    ...
    public void postReply(){
        ...
        HttpUtil.post(CommonData.groupURL,paramsPost,new JsonHttpResponseHandler(){
            ...
            public void onSuccess(int statusCode, Header[] headers, org.json.JSONObject response) {
                ...
                Toast.makeText(SiteArticleActivity.this, "回复成功",Toast.LENGTH_SHORT).show();
                HttpUtil.post(CommonData.groupURL, params, new TextHttpResponseHandler() {
                    ...
                    public void onSuccess(int statusCode, Header[] headers, String responseString) {
                        ...
                        //Article article = JsonParse.parseArticle(resp);
                        //List<Tag> tagList = article.getG_a_tags();
                        List<SiteReply> replyList = JsonParse.parseSReplys(resp);
